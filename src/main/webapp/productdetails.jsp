...
<%
	List<mobileentity> ml1 = (List<mobileentity>) session.getAttribute("mlist");
	mobileentity l = ml1.get(0);
	%>
<head>
...
function addcart(){
	$.ajax({
	  url: 'AddToCart?username=<%=session.getAttribute("username")%>&productid=<%=l.getProductid()%>',
	  method     : 'GET',
	});
...
<script>
function incart(){
	document.getElementById('icart').innerHTML='In cart';
}
</script>
</head>
<body>
			...
			<div class="row">
				<div class="col-md-6">
					<div id="slider" class="owl-carousel product-slider">
						<div class="item">
							<img style="object-fit: contain; height: 400px;"
								src="images/mobiles/<%=l.getBimg1()%>" />
						</div>
						<%
						if (l.getVideo() != null) {
						%>
						<div class="item">
							<div
								style="object-fit: contain; height: 400px; width: 400px; padding: 15px;">

								<iframe width="676" height="400" src="<%=l.getVideo()%>"
									title="iPhone 12" frameborder="0"
						...
						<div class="item">
							<img style="object-fit: contain; height: 400px;"
								src="images/mobiles/<%=l.getBimg2()%>" />
						</div>
						<div class="item">
							<img style="object-fit: contain; height: 400px;"
								src="images/mobiles/<%=l.getBimg3()%>" />
						</div>
						<div class="item">
							<img style="object-fit: contain; height: 400px;"
								src="images/mobiles/<%=l.getBimg4()%>" />
						</div>
						<div class="item">
							<img style="object-fit: contain; height: 400px;"
								src="images/mobiles/<%=l.getBimg5()%>" />
						</div>
					...
					<div id="thumb" class="owl-carousel product-thumb pt-2"
						style="height: 150px;">
						<div class="item">
							<img style="object-fit: contain; height: 100px;"
								src="images/mobiles/<%=l.getBimg1()%>" />
						</div>
						<%
						if (l.getVideo() != null) {
						...
						<div class="item">
							<img
								style="object-fit: contain; height: 100px; filter: grayscale(100%);"
								src="images/mobiles/<%=l.getBimg1()%>" />
						</div>
						<%
						}
				...
				<div class="col-md-6">
					<div class="product-dtl">
						<div class="product-info">
							<div class="product-name"><%=l.getProduct_name()%></div>
							<div class="reviews-counter">
								<div class="rate">
									<input type="radio" id="star5" name="rate" value="5" checked />
									<label for="star5" title="text">5 stars</label> <input
										type="radio" id="star4" name="rate" value="4" checked />
									<label for="star4" title="text">4 stars</label> <input
										type="radio" id="star3" name="rate" value="3" checked />
							...
							<div class="product-price-discount">
								<span><i class="fa-solid fa-indian-rupee-sign"></i> <%=l.getOff_price()%></span><span
									class="line-through"><i
									class="fa-solid fa-indian-rupee-sign"></i><%=l.getPrice()%></span>
							</div>
						</div>
						<p>
						<div class="text-secondary" style="">
							highlights:
							<ul style="">
								<li class=""><%=l.getStorage()%></li>
								<li class=""><%=l.getDisplay()%></li>
								<li class=""><%=l.getCamara()%></li>
								<li class=""><%=l.getBattery()%></li>
								<li class=""><%=l.getProcessor()%></li>
							</ul>
						...
						<div class="row">
							<div class="col-md-6">
								<label for="size">Variant</label> <select id="size" name="size"
									class="form-control" disabled>
									<option><%=l.getStorage()%></option>
									<option selected>4gb,128gb</option>
							...
							<div class="col-md-6">
								<label for="color">Color</label> <select id="color" name="color"
									class="form-control" disabled>
									<option selected><%=l.getColor()%></option>
									<option>Green</option>
									<option>Red</option>
								</select>
							</div>
						...
						<div class="product-count">
							...
							<% if((session.getAttribute("username")!= null)&&(d.incart((String)session.getAttribute("username"), l.getProductid()))) 
							{ %>
								<a 
								href="ViewCart?username=<%= session.getAttribute("username") %>" 
								class="round-black-btn">In Cart</a>
							<% }else{ %>
							<a <%if (session.getAttribute("username") != null) {%>
								href="AddToCart?username=<%=session.getAttribute("username")%>&productid=<%=l.getProductid()%>"  
								<%} else {%> href="login.jsp" <%}%> class="round-black-btn" id="icart" >Add to Cart</a>
								<% } %>
